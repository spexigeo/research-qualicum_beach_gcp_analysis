...
4. Export GCPs for MetaShape
5. Calculate H3 cells overlapping the region

## Installation

```bash
cd research-qualicum_beach_gcp_analysis
pip install -e .
```
...
- Supports Point and Polygon placemarks
...
The `gcp_exporter` module exports GCPs for use in MetaShape:

- CSV format (tab-separated) for marker import
- XML format (marker file) for alternative import
- Compatible with Agisoft MetaShape workflow

### H3 Cell Calculation

The `visualization` module can calculate H3 cells overlapping a bounding box:

- Supports various H3 resolutions
- Useful for spatial indexing and analysis
...
```
research-qualicum_beach_gcp_analysis/
├── qualicum_beach_gcp_analysis/
│   ├── __init__.py
│   ├── kmz_parser.py          # KMZ file parsing
│   ├── basemap_downloader.py  # Basemap tile downloading
│   ├── visualization.py       # GCP visualization and H3 cells
│   └── gcp_exporter.py        # MetaShape export
